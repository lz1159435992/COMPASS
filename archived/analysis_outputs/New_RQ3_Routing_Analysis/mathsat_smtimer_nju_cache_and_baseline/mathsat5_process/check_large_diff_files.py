...
def check_large_diff_files(analysis_file, log_dir):
    """
    找出差距在10秒以上的文件，并检查从日志文件中提取的数据是否正确
    ...
    for file_data in analysis_data['details']:
        ...
        # 计算差距
        diff = abs(info_dict_time - log_calculated_time)
        ...
        if diff >= 10:
            ...
            large_diff_files.append(verification_result)
    
    # 按差距排序
    large_diff_files.sort(key=lambda x: x['difference'], reverse=True)
    
    # 打印结果
    print(f"\n找到 {len(large_diff_files)} 个差距在10秒以上的文件")
    
    # 打印验证结果统计
    verification_stats = {'correct': 0, 'incorrect': 0, 'not_found': 0}
    for file_info in large_diff_files:
    ...
    # 打印差距最大的10个文件
    print("\n差距最大的10个文件:")
    for i, file_info in enumerate(large_diff_files[:10]):
        ...
        if file_info['verification'] != 'not_found':
            ...
            if 'verified_line_302_count' in file_info:
                print(f"     line_302 (变量相关断言求解成功): {file_info['verified_line_302_count']}次, 总计{file_info['verified_line_302_sum']:.3f}秒")
            if 'verified_line_304_count' in file_info:
                print(f"     line_304 (变量相关断言求解失败: unsat/timeout): {file_info['verified_line_304_count']}次, 总计{file_info['verified_line_304_sum']:.3f}秒")
    ...
    # 保存结果到文件
    output_file = 'large_diff_files_check.json'
    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump(large_diff_files, f, indent=2)
...
def main():
    ...
    主函数
    """
    parser = argparse.ArgumentParser(description='找出差距在10秒以上的文件，并检查从日志文件中提取的数据是否正确')
    ...
    check_large_diff_files(args.analysis_file, args.log_dir)
