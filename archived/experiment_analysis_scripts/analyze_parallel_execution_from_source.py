#!/usr/bin/env python3
"""
RQ3 并行执行分析脚本 - 基于原始数据源

数据源:
1. QF_NIA_test.json: 测试对象列表 (10,043 个约束)
...
注意:
- COMPASS 文件中的 SAT 案例大多是直接求解已经能解决的
- 真正的 unknown -> sat 转换需要从 QF_NIA_advanced_solver_results_all.json 分析
"""
...
def analyze_parallel_execution():
    ...
    stats = {
        ...
        'parallel': {
            ...
            'sat_times': [], 'unsat_times': [], 'unknown_times': [],
            'total_time': 0,
            'compass_wins': 0,  # COMPASS 先完成的次数
            'direct_wins': 0    # 直接求解先完成的次数
        }
    }
    
    # 详细分析
    compass_improvements = []  # COMPASS 改进的案例
    ...
    for test_key in test_data.keys():
        ...
        if direct_status == 'sat':
            ...
            stats['direct']['unknown_times'].append(TIMEOUT)  # unknown 计为超时
        
        stats['direct']['total_time'] += direct_time if direct_status in ['sat', 'unsat'] else TIMEOUT
        
        # 获取 COMPASS 结果
        compass_result = compass_normalized.get(norm_key)
        
        # 并行执行逻辑
        if compass_result and isinstance(compass_result, list) and len(compass_result) >= 4:
            compass_status = str(compass_result[0]).lower() if compass_result[0] else 'unknown'
            compass_exec_time = compass_result[3] if isinstance(compass_result[3], (int, float)) else TIMEOUT
            compass_exec_time = min(compass_exec_time, TIMEOUT)  # 限制不超过 TIMEOUT
            
            # 并行执行: 谁先完成用谁的结果
            if direct_status in ['sat', 'unsat']:
                # 直接求解已经有结果
                if compass_status == 'sat' and compass_exec_time < direct_time:
                    # COMPASS 更快且得到 SAT
                    parallel_status = 'sat'
                    parallel_time = compass_exec_time
                    stats['parallel']['compass_wins'] += 1
                else:
                    # 使用直接求解结果
                    parallel_status = direct_status
                    parallel_time = direct_time
                    stats['parallel']['direct_wins'] += 1
            else:
                # 直接求解是 unknown
                if compass_status == 'sat':
                    ...
                    parallel_time = compass_exec_time
                    stats['parallel']['compass_wins'] += 1
                    compass_improvements.append({
                        'key': norm_key[-60:],
                        'compass_time': compass_exec_time,
            ...
            # 没有 COMPASS 结果，使用直接求解结果
            parallel_status = direct_status
            parallel_time = direct_time if direct_status in ['sat', 'unsat'] else TIMEOUT
            stats['parallel']['direct_wins'] += 1
        ...
        stats['parallel']['total_time'] += parallel_time if parallel_status in ['sat', 'unsat'] else TIMEOUT
    
    return stats, compass_improvements

def print_analysis(stats, compass_improvements):
    """打印分析结果"""
    ...
    if d['sat_times']:
        print(f"  SAT 平均时间: {sum(d['sat_times'])/len(d['sat_times']):.1f}s")
    if d['unsat_times']:
        print(f"  UNSAT 平均时间: {sum(d['unsat_times'])/len(d['unsat_times']):.1f}s")
    ...
    if p['sat_times']:
        print(f"  SAT 平均时间: {sum(p['sat_times'])/len(p['sat_times']):.1f}s")
    if p['unsat_times']:
        print(f"  UNSAT 平均时间: {sum(p['unsat_times'])/len(p['unsat_times']):.1f}s")
    ...
    print(f"\n  COMPASS 获胜次数: {p['compass_wins']}")
    print(f"  直接求解获胜次数: {p['direct_wins']}")
    ...
    print(f"  SAT 增加: +{sat_improvement} ({100*sat_improvement/d['sat']:.1f}% 提升)")
    ...
    # COMPASS 改进的案例
    if compass_improvements:
        print(f"\n--- COMPASS 改进的案例 (共 {len(compass_improvements)} 个) ---")
        avg_compass_time = sum(c['compass_time'] for c in compass_improvements) / len(compass_improvements)
        print(f"  平均 COMPASS 执行时间: {avg_compass_time:.1f}s")
...
""")
    ...
    return {
        'total': total,
        'direct_sat': d['sat'],
        'direct_unsat': d['unsat'],
        'direct_unknown': d['unknown'],
        'parallel_sat': p['sat'],
        'parallel_unsat': p['unsat'],
        'parallel_unknown': p['unknown'],
        'sat_improvement': sat_improvement,
        'time_saved_hours': time_saved / 3600,
        'compass_improvements': len(compass_improvements)
    }

if __name__ == "__main__":
    stats, compass_improvements = analyze_parallel_execution()
    if stats:
        results = print_analysis(stats, compass_improvements)
