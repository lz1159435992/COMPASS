...
def test_group():
    ...
    if not os.path.exists(info_name):
        ...
        print(f'文件已存在。')
    with open(os.path.join(config.TEST_RL_ROOT, 'info_dict_gai_6_normal_1109_pre_SMTimer_llama3.1:70b_1200s.txt'), 'r') as file:
...
def test_group_1_save():
    solve_name = os.path.join(config.get_data_dir('solve'), 'info_dict.txt')
    solve_dict = load_dictionary(solve_name)

    info_name = os.path.join(config.TEST_RL_ROOT, 'info_dict_gai_6_normal_1109_pre_SMTimer_save_docker_llama_3.1:70b_1200s.txt')
    info_dict = load_dictionary(info_name)
    for k,v in info_dict.items():
...
def test_group_2_save_1208(solve_name,info_name):
    ...
    # info_name = os.path.join(config.TEST_RL_ROOT, 'info_dict_gai_6_normal_1109_pre_SMTimer_llama3.1:70b_1200s.txt')
...
def test_group_1_no_save():
    solve_name = config.get_baseline_path('info_dict_bingxing')
    solve_dict = load_dictionary(solve_name)
...
def test_group_1_llm():
    ...
    info_name = os.path.join(config.TEST_RL_ROOT, 'info_dict_normal_1109_llm_no_rl_direct_solve_docker_llama3.1:70b_1set_1200s.txt')
    info_dict = load_dictionary(info_name)
    for k,v in info_dict.items():
        if k in solve_dict.keys():
            info_dict[k][0] = solve_dict[k][0]
...
def test_group_3():
    ...
    info_name = os.path.join(config.get_data_dir('solve'), 'info_dict.txt')
    if not os.path.exists(info_name):
        # 文件不存在时，创建文件
        info_dict = {}
        with open(info_name, 'w') as file:
...
def something():
    ...
    # 定义文件路径列表
    file_paths = [
        os.path.join(config.TEST_RL_ROOT, 'info_dict_normal_1103_llm_no_rl_direct_solve_docker_llama3.1:70b_1set.txt'),
        config.get_external_file('info_dict_rl'),
        os.path.join(config.TEST_RL_ROOT, 'info_dict_gai_6_normal_109_2_SMTimer.txt'),
        # os.path.join(config.get_data_dir('solve'), 'info_dict.txt')
    ]
    ...
    solve_name = os.path.join(config.TEST_RL_ROOT, 'info_dict_normal_1109_llm_no_rl_direct_solve_docker_llama3.1:70b_1set_1200s.txt')
    solve_dict = load_dictionary(solve_name)
    ...
    info_dict = load_dictionary(os.path.join(config.TEST_RL_ROOT, 'result_dict.txt'))
    print(len(info_dict))
def test_group_2_no_save(solve_name,info_name):
    ...
    #修改求解时间,求解时间大于1200s的测试数据记为unknown
def test_group_2_no_save_1207(solve_name, info_name, var_count_path=os.path.join(project_root, 'test_rl', 'test_solve', 'var_count.txt')):
    time_dict = {}
    time_dict_2 = {}
    solve_dict = load_dictionary(solve_name)
...
def test_group_2_no_save_0607_cvc5(solve_name,info_name,new_solver):
    ...
    var_count = load_dictionary(os.path.join(config.TEST_RL_ROOT, 'test_solve', 'var_count.txt'))
    # info_name = os.path.join(config.TEST_RL_ROOT, 'info_dict_gai_6_normal_1109_pre_SMTimer_llama3.1:70b_1200s.txt')
...
def resolve_dataset():
    result_dict = {}
    result_dict_2 = {}
    solve_dict = load_dictionary(config.get_baseline_path('info_dict_bingxing'))
...
def resolve_dataset_300s():
    ...
    with open('result_dict_z3solver_300s.txt', 'w') as file:
        json.dump(result_dict_2, file, indent=4)
def resolve_dataset_cvc5_smtimer(file_path=None):
    if file_path is None:
        file_path = os.path.join(project_root, 'test_rl', 'smtimer_experiments', 'cvc5_smtimer_results.json')
    result_dict = {}
    result_dict_2 = {}
    ...
    unknown_count = 0
    unknown_time = 0
    var_count = load_dictionary(os.path.join(project_root, 'test_rl', 'test_solve', 'var_count.txt'))
    for k,v in solve_dict.items():
...
def resolve_time_dict_300s_RL_LLM(data_dict):
    var_count = load_dictionary(os.path.join(project_root, 'test_rl', 'test_solve', 'var_count.txt'))
    with open(os.path.join(project_root, 'test_rl', 'smtimer_experiments', 'info_dict_rl.txt'), 'r') as file:
        rl_dict = json.load(file)
    result_dict = {}
    result_dict_2 = {}
    #收集每个类别下的求解时间
...
def resolve_dataset_by_var_count():
    ...
    with open(os.path.join(project_root, 'test_rl', 'smtimer_experiments', 'info_dict_rl.txt'), 'r') as file:
        rl_dict = json.load(file)
    solve_dict = load_dictionary(os.path.join(project_root, 'test_rl', 'test_solve', 'info_dict_bingxing.txt'))
    for k, v in solve_dict.items():
        if v[0] =='sat' and v[1] > 1200:
            solve_dict[k][0] = 'unknown'
            solve_dict[k][1] = 1200
...
def test_group_cvc5_process_analysis(info_name, var_count_path=os.path.join(project_root, 'test_rl', 'test_solve', 'var_count.txt'),
...
if __name__ == '__main__':
    ...
    #获取变量个数 cvc5 smtimer
    # get_var_count_from_dataset('/home/<USER>/PycharmProjects/Pearl/test_rl/smtimer_experiments/cvc5_process/info_dict_SMTimer_llama3.1:70b_1200s_info_dict_rl_cvc5_0628.txt', 'cvc5_smtimer_var_count.txt')
    # test_group()
    # test_group_1()
    # test_group_2()
    # test_group_3()
    ...
    # something()
    #z3结果smtimer
    # resolve_dataset_cvc5_smtimer('/home/<USER>/PycharmProjects/Pearl/test_rl/smtimer_experiments/z3_smtimer_results.json')
    # #cvc5结果smtimer
    ...
    # 示例调用，包含SuperVenn图生成
    output_dir = os.path.join(project_root, 'test_rl', 'smtimer_experiments', 'cvc5_process', 'supervenn_output')
    ...
    #mathsat结果smtimer
    # resolve_dataset_cvc5_smtimer('/home/<USER>/PycharmProjects/Pearl/test_rl/smtimer_experiments/mathsat5_smtimer_results.json')
    ...
    # solve_name = config.get_baseline_path('info_dict_bingxing')
    # info_name = os.path.join(config.TEST_RL_ROOT, 'info_dict_gai_6_normal_1110_pre_SMTimer_llama3.1:70b_1200s_info_dict_rl.txt')
    # new_solver = '/home/<USER>/PycharmProjects/Pearl/test_rl/smtimer_experiments/cvc5_smtimer_results.json'
